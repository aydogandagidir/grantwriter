...
--
-- Depends on the helper functions migration (auth.tenant_id /
-- auth.is_tenant_admin must exist) and on the soft-delete column added
...
create policy proposals_tenant_insert on proposals
  ...
  with check (tenant_id = auth.tenant_id() and created_by = auth.uid());
...
create policy proposals_tenant_delete on proposals
  ...
  using (tenant_id = auth.tenant_id() and auth.is_tenant_admin());
